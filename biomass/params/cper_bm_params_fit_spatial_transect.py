...
def load_df(inPATH, date_cols=[date_col]):
    ...
    past_attributes['Plot'] = past_attributes.apply(lambda x: '_'.join([x['PastureCode'], 
                                                                      'P'+str(x['Plot'])]),
                                                  axis=1)
    ...
    df = pd.merge(df, past_attributes[['Plot', 'Ecosite', 'Spatial']], on='Plot', how='left')
